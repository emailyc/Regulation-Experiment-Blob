...
jsPsych.plugins["button-response-blobs"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    trial.prompt = (typeof trial.prompt === 'undefined') ? "" : trial.prompt;
	  //trial.dogsleft = trial.dogsleft;

//alert(jsPsych.data.getLastTrialData().rt)
//alert("getlasttrial: "+jsPsych.data.getLastTrialData().dogleft)
    // if any trial variables are functions
    // this evaluates the function and replaces
    // it with the output of the function
    trial = jsPsych.pluginAPI.evaluateFunctionParameters(trial);

    //alert (trial.button_html)
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      ...
      $('#jspsych-button-response-btngroup').append(
        $(str).attr('id', 'jspsych-button-response-button-' + i).data('choice', i).addClass('jspsych-button-response-button').on('click', function(e) {
         if((jsPsych.data.getLastTrialData().numleft1<1)){
         	alert("None left in this category, please choose another category")
         }else{
         	//alert("dog>0")
      	...
      	$('#jspsych-button-response-btngroup').append(
        $(str).attr('id', 'jspsych-button-response-button-' + i).data('choice', i).addClass('jspsych-button-response-button').on('click', function(e) {
          if((jsPsych.data.getLastTrialData().numleft2<1)){
            alert("None left in this category, please choose another category")
         }else{
         	//alert("cat>0")
          var choice = $('#' + this.id).data('choice');
      	...
      	$('#jspsych-button-response-btngroup').append(
        $(str).attr('id', 'jspsych-button-response-button-' + i).data('choice', i).addClass('jspsych-button-response-button').on('click', function(e) {
          if((jsPsych.data.getLastTrialData().numleft3<1)){
            alert("None left in this category, please choose another category")
         }else{
         	//alert("bird>0")
          var choice = $('#' + this.id).data('choice');
      	...
      	$('#jspsych-button-response-btngroup').append(
        $(str).attr('id', 'jspsych-button-response-button-' + i).data('choice', i).addClass('jspsych-button-response-button').on('click', function(e) {
          if((jsPsych.data.getLastTrialData().numleft4<1)){
            alert("None left in this category, please choose another category")
         }else{
         	//alert("rabbit>0")
          var choice = $('#' + this.id).data('choice');
